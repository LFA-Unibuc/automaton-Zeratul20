    ...
    def validate(self):
        ...
        while line.strip() != "End":
            if len(line.strip().split()) == 1:
                ...
                self.states.append(line.split(",")[0].strip())
                if line.strip().split(",")[1] == "F" and line.strip().split(",")[2] == "S" or line.strip().split(",")[
                    1] == "S" and line.strip().split(",")[2] == "F":
                    nrF = nrF + 1
                    nrS = nrS + 1
                else:
                    f.close()
        ...
        while line.strip() != "End":
            if len(line.strip().split(",")) == 3:
                if line.split(",")[0].strip() not in self.states or line.split(",")[2].strip() not in self.states or \
                        line.split(",")[1].strip() not in self.words:
                    f.close()
                    raise ValidationException
            else:
                f.close()
